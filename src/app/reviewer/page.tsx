...
export default function ReviewerDashboard() {
  const { user } = useAuth();
  const { reviews, isLoadingReviews, reviewsError, getUserReviews } =
    useReviewer();
  ...
  const stats = {
    ...
    assigned: reviews.filter(
      (r) => r.status === "IN_PROGRESS" && !r.recommendation,
    ).length, // IN_PROGRESS without recommendation = waiting for review
                    ...
                    <tr key={review.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-3">
                          {(review.status as string) === "IN_PROGRESS" &&
                            !review.recommendation && (
                              <Link
