...
export default function AdminStartupsPage() {
  ...
  const filteredStartups = users.filter(startup => {
    ...
    const fullName = `${startup.person?.firstName || startup.firstName || ""} ${startup.person?.lastName || startup.lastName || ""}`.toLowerCase();
  ...
  const handleApprove = async (startup: AdminUser) => {
    showToast.success(`Approved startup application for ${startup.person?.firstName || startup.firstName}`);
    // TODO: Implement actual approval logic
  };

  const handleReject = async (startup: AdminUser) => {
    showToast.error(`Rejected startup application for ${startup.person?.firstName || startup.firstName}`);
        ...
        <AdminLayout>
          <div className="space-y-6 md:space-y-8 p-4 sm:p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Startup Management</h1>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1">
                  <label htmlFor="search" className="sr-only">Search startups</label>
                ...
                {isLoading ? (
                  ...
                  <div className="divide-y divide-gray-200">
                    {filteredStartups.map((startup) => (
                      <div key={startup.personId} className="p-4">
                          ...
                          <div className="flex items-center space-x-3">
                            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-100 border-2 border-blue-200">
                              <span className="text-sm font-semibold text-blue-700">
                                {startup.person?.firstName?.charAt(0) || startup.firstName?.charAt(0) || "?"}
                                {startup.person?.lastName?.charAt(0) || startup.lastName?.charAt(0) || ""}
                              </span>
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-medium text-gray-900 truncate">
                                {startup.person?.firstName || startup.firstName || "Unknown"} {startup.person?.lastName || startup.lastName || ""}
                              </p>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {isLoading ? (
                      ...
                      filteredStartups.map((startup) => (
                        <tr key={startup.personId} className="hover:bg-gray-50">
