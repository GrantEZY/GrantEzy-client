...
export default function ProgramDetailsPage() {
  ...
  useEffect(() => {
    if (cycles && cycles.length > 0) {
      const program = cycles.find(
        (cycle) => cycle.program?.id === programId,
      )?.program;
      if (program) {
  ...
  const programCycles = cycles.filter(
    (cycle) => cycle.program?.id === programId,
  );
            ...
            <div className="rounded-lg border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <button
                    onClick={() => router.push("/pm/programs")}
                    className="bg-opacity-60 hover:bg-opacity-80 flex-shrink-0 rounded-lg bg-white p-2 transition-colors"
                  >
                    <svg
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">
                          {currentProgram.details?.description ||
