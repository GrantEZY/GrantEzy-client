...
export default function PMReviewDetailsPage() {
  ...
  const getStatusBadgeClass = (status: ReviewStatus | string) => {
    if (status === ReviewStatus.COMPLETED || status === "COMPLETED") {
      return "bg-green-100 text-green-800";
    } else if (status === ReviewStatus.IN_PROGRESS || status === "IN_PROGRESS") {
        ...
        <div className="space-y-6">
          ...
          <div>
            <button
              ...
              onClick={() => router.push(`/pm/cycles/${cycleSlug}/applications/${applicationSlug}`)}
              type="button"
            >
              <svg
          ...
          {!isReviewLoading && !reviewError && currentReview && (
            <>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-4">
                      <h2 className="text-xl font-semibold text-gray-900">
                        Review by {currentReview.reviewer?.firstName && currentReview.reviewer?.lastName 
                          ? `${currentReview.reviewer.firstName} ${currentReview.reviewer.lastName}`
                          : currentReview.reviewer?.email || "Anonymous Reviewer"}
                      </h2>
                      <span
                        className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${getStatusBadgeClass(currentReview.status)}`}
                      >
                        {currentReview.status === ReviewStatus.COMPLETED || (currentReview.recommendation && currentReview.scores) 
                          ? "COMPLETED" 
                          : "IN_PROGRESS"}
                      </span>
                    </div>
              ...
              {currentReview.recommendation && (
                <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      {currentReview.recommendation === Recommendation.APPROVE && "✓ APPROVE"}
                      {currentReview.recommendation === Recommendation.REJECT && "✗ REJECT"}
                      {currentReview.recommendation === Recommendation.REVISE && "↻ REVISE"}
                    </span>
                    <p className="text-sm text-gray-600">
                      {currentReview.recommendation === Recommendation.APPROVE && 
                    ...
                    <div className="border-b border-gray-100 pb-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900">Technical Feasibility</h4>
                            <p className="text-xs text-gray-500">Assesses the technical viability and implementation approach</p>
                          </div>
                        </div>
                        <div className={`rounded-lg px-4 py-2 ${getScoreColor(currentReview.scores.technical)}`}>
                          <p className="text-2xl font-bold">{currentReview.scores.technical}</p>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${currentReview.scores.technical}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="border-b border-gray-100 pb-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900">Market Potential</h4>
                            <p className="text-xs text-gray-500">Evaluates market opportunity and competitive positioning</p>
                          </div>
                        </div>
                        <div className={`rounded-lg px-4 py-2 ${getScoreColor(currentReview.scores.market)}`}>
                          <p className="text-2xl font-bold">{currentReview.scores.market}</p>
                          <p className="text-xs">/ 100</p>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                    ...
                    <div className="border-b border-gray-100 pb-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900">Financial Viability</h4>
                            <p className="text-xs text-gray-500">Reviews budget, revenue model, and financial sustainability</p>
                          </div>
                        </div>
                        <div className={`rounded-lg px-4 py-2 ${getScoreColor(currentReview.scores.financial)}`}>
                          <p className="text-2xl font-bold">{currentReview.scores.financial}</p>
                          <p className="text-xs">/ 100</p>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${currentReview.scores.financial}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="border-b border-gray-100 pb-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900">Team Capability</h4>
                            <p className="text-xs text-gray-500">Assesses team expertise, experience, and execution capacity</p>
                          </div>
                        </div>
                        <div className={`rounded-lg px-4 py-2 ${getScoreColor(currentReview.scores.team)}`}>
                          <p className="text-2xl font-bold">{currentReview.scores.team}</p>
                          <p className="text-xs">/ 100</p>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                    ...
                    <div className="pb-2">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-gray-900">Innovation & Impact</h4>
                            <p className="text-xs text-gray-500">Evaluates novelty, differentiation, and potential impact</p>
                          </div>
                        </div>
                        <div className={`rounded-lg px-4 py-2 ${getScoreColor(currentReview.scores.innovation)}`}>
                          <p className="text-2xl font-bold">{currentReview.scores.innovation}</p>
                          <p className="text-xs">/ 100</p>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${currentReview.scores.innovation}%` }}
                        ></div>
                      </div>
                    </div>

                    {/* Overall Score */}
                    <div className="mt-6 rounded-lg bg-gradient-to-r from-indigo-50 to-blue-50 p-4 border-2 border-indigo-200">
                      <div className="flex items-center justify-between">
                        <div>
                          <h4 className="text-lg font-bold text-gray-900">Overall Average Score</h4>
                          <p className="text-sm text-gray-600">Calculated from all evaluation criteria</p>
                        </div>
                        <div className="text-right">
                          <p className="text-4xl font-bold text-indigo-600">
                            {((currentReview.scores.technical + currentReview.scores.market + currentReview.scores.financial + currentReview.scores.team + currentReview.scores.innovation) / 5).toFixed(1)}
                          </p>
              ...
              {currentReview.suggestedBudget && (
                <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">Suggested Budget</h3>
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-gray-900">
                        {currentReview.suggestedBudget.currency} {currentReview.suggestedBudget.amount.toLocaleString()}
                      </p>
                  ...
                  <div className="space-y-2">
                    {currentReview.application.title && (
                      <div>
                        ...
                        <p className="text-sm text-gray-900">{currentReview.application.title}</p>
                      </div>
                    )}
                    <div className="grid grid-cols-2 gap-4 mt-4">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Application ID</p>
                        <p className="text-xs text-gray-700 font-mono">{currentReview.applicationId}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-600">Review ID</p>
                        <p className="text-xs text-gray-700 font-mono">{currentReview.id}</p>
                      </div>
                    </div>
                  </div>
                </div>
