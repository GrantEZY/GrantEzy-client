    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {criteriaSubmissions.map((submission: any) => {
          ...
          return (
            <div key={submission.id} className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
              {/* Submission Header */}
                    ...
                    <div className="mt-2 flex items-center space-x-4 text-sm text-gray-600">
                      <div className="flex items-center">
                        ...
                        {submission.project?.application?.applicant?.person?.firstName && submission.project?.application?.applicant?.person?.lastName
                          ? `${submission.project.application.applicant.person.firstName} ${submission.project.application.applicant.person.lastName}`
                          : submission.project?.application?.applicant?.contact?.email || 'No applicant info'}
                      </div>
                      <div className="flex items-center">
                        <svg className="mr-1.5 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {isExpanded && (
                <div className="p-6 space-y-6">
                  {/* Submission Details */}
                  <div>
                    <h5 className="text-sm font-semibold text-gray-900 mb-3">Assessment Statement</h5>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        onClick={() => handleInviteReviewer(submission.id, submission.project?.application?.basicDetails?.title || submission.project?.title || 'Project')}
                    ...
                    {reviews.length === 0 ? (
                      <div className="text-center py-8 bg-gray-50 rounded-lg">
                        ...
                        <p className="mt-1 text-sm text-gray-500">Invite a reviewer to evaluate this assessment.</p>
                      </div>
                    ) : (
                              ...
                              <div className="flex-1">
                                <div className="flex items-center space-x-3">
                                  ...
                                  <span className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${
                                    review.status === 'COMPLETED' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                                  }`}>
                                    {review.status}
                                  </span>
                                  {review.recommendation && getRecommendationBadge(review.recommendation)}
                                </div>
                                
                                {review.reviewAnalysis && review.status === 'COMPLETED' && (
                                  <div className="mt-3 bg-gray-50 rounded p-3">
                                    <p className="text-xs font-medium text-gray-700 mb-1">Analysis:</p>
                                    <p className="text-sm text-gray-600 whitespace-pre-wrap">{review.reviewAnalysis}</p>
                                  </div>
                                )}
                              </div>
