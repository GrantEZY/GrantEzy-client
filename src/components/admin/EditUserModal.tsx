...
export function EditUserModal({ isOpen, onClose, onSubmit, isLoading, user }: EditUserModalProps) {
  ...
  useEffect(() => {
    if (user && isOpen) {
      ...
      } else if (user.role && Object.values(UserRoles).includes(user.role as UserRoles)) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/10 backdrop-blur-md">
      <div className="mx-4 w-full max-w-md scale-100 transform rounded-lg bg-white shadow-xl transition-all duration-200">
        ...
        <form className="space-y-4 px-6 py-4" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Current Roles</label>
            ...
            <p className="mt-1 text-xs text-gray-500">
              Current roles assigned to this user. Click × to remove a role (minimum 1 role
              required).
            </p>
          </div>

          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">Manage Roles</label>
