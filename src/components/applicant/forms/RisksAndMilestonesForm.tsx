...
import { Milestone, Risk, RiskImpact } from "@/types/applicant.types";
...
export default function RisksAndMilestonesForm() {
  ...
  const addRisk = () => {
    setRisks([
      ...risks,
      { description: "", impact: RiskImpact.MEDIUM, mitigation: "" },
    ]);
  };
  ...
  const addMilestone = () => {
    setMilestones([
      ...milestones,
      { title: "", description: "", deliverables: [], dueDate: "" },
    ]);
  };
  ...
  const updateDeliverable = (
    ...
    const milestone = milestones[milestoneIndex];
    const newDeliverables = milestone.deliverables.map(
      (d: string, i: number) => (i === deliverableIndex ? value : d),
    );
  ...
  const validateForm = (): boolean => {
    ...
    risks.forEach((risk: Risk, index: number) => {
      ...
      if (!risk.mitigation.trim()) {
        newErrors[`risk_${index}_mitigation`] =
    ...
    milestones.forEach((milestone: Milestone, index: number) => {
      ...
      if (!milestone.description.trim()) {
        newErrors[`milestone_${index}_description`] =
          "Milestone description is required";
      }
      if (milestone.deliverables.length === 0) {
        newErrors[`milestone_${index}_deliverables`] =
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4 rounded-lg border border-gray-200 p-6">
        ...
        {errors.risksRequired && (
          <p className="text-sm text-red-500">{errors.risksRequired}</p>
        )}
            ...
            >
              ...
              <div>
                ...
                <select
                  value={risk.impact}
                  onChange={(e) =>
                    updateRisk(index, "impact", e.target.value as RiskImpact)
                  }
              ...
              <div>
                ...
                {milestone.deliverables.map(
                  (deliverable: string, dIndex: number) => (
                    <div key={dIndex} className="mb-2 flex gap-2">
                      <input
                        type="text"
                        value={deliverable}
                        onChange={(e) =>
                          updateDeliverable(index, dIndex, e.target.value)
                        }
