...
export default function DocumentsForm() {
  const { addDocuments, isLoading, goToPreviousStep, currentApplication } =
    useApplicant();

  const [documents, setDocuments] = useState<DocumentUpload>({
    endorsementLetter: currentApplication?.documents?.endorsementLetter || null,
    plagiarismUndertaking:
      currentApplication?.documents?.plagiarismUndertaking || null,
  ...
  const handleDocumentUpload = (
    ...
    const document: Document = {
      ...
      description:
        REQUIRED_DOCUMENTS.find((d) => d.key === field)?.description || "",
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          Upload all required documents for your application. Files will be
          securely stored.
        </p>
      </div>
          ...
          >
            <FileUpload
              ...
              onUploadSuccess={(result) =>
                handleDocumentUpload(doc.key, result)
              }
              onUploadError={(error) =>
                setErrors((prev) => ({ ...prev, [doc.key]: error }))
              }
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <button
          ...
          className="inline-flex items-center rounded-md border border-gray-300 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50"
        >
          <svg
        ...
        <div className="flex items-center space-x-4">
          ...
          <button
            ...
            className="inline-flex items-center rounded-md bg-blue-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
