...
export function CycleDetailsModal({ isOpen, onClose, cycle }: CycleDetailsModalProps) {
  ...
  useEffect(() => {
    if (isOpen && cycle) {
      ...
      if (cycle.slug) {
        ...
        console.warn('Cycle does not have a slug, cannot fetch cycle details:', cycle);
      }
    }
          ...
          {isProgramCyclesLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <span className="ml-2 text-sm font-semibold text-gray-900">
                        {formatCurrency(currentCycle.budget.amount, currentCycle.budget.currency)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                  <h3 className="mb-3 text-lg font-semibold text-gray-900">Duration</h3>
                  <div className="space-y-3">
                    <div>
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-6">
                <h3 className="mb-4 text-lg font-semibold text-gray-900">TRL Criteria</h3>
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                  {Object.entries(currentCycle.trlCriteria || {}).map(([trl, criteria]) => (
                    <div key={trl} className="rounded-lg border border-gray-100 bg-gray-50 p-4">
                      <h4 className="mb-3 font-medium text-gray-900">{trl}</h4>
              ...
              {/* Scoring Scheme */}
              <div className="rounded-lg border border-gray-200 bg-white p-6">
                <h3 className="mb-4 text-lg font-semibold text-gray-900">Scoring Scheme</h3>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(currentCycle.scoringScheme || {}).map(([category, criteria]) => (
                    <div
                      key={category}
                      className="rounded-lg border border-gray-100 bg-gray-50 p-4"
                    >
                      <h4 className="mb-3 font-medium text-gray-900 capitalize">{category}</h4>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-500">Min Score:</span>
                          <span className="text-gray-900">{criteria.minScore}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Max Score:</span>
                          <span className="text-gray-900">{criteria.maxScore}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">Weightage:</span>
                          <span className="text-gray-900">{criteria.weightage}%</span>
                        </div>
                      </div>
                    </div>
                  ))}
